...
{
    ESP_LOGI(TAG,"bsp_audio_codec_speaker_init");
...
{
    ESP_LOGI(TAG,"bsp_audio_codec_microphone_init");
    const audio_codec_data_if_t *i2s_data_if = bsp_audio_get_codec_itf();
    if (i2s_data_if == NULL) {
        /* Initilize I2C */
        BSP_ERROR_CHECK_RETURN_NULL(bsp_i2c_init());
...
{
    ESP_LOGI(TAG,"bsp_io_expander_init");

    /* Initilize I2C */
    BSP_ERROR_CHECK_RETURN_NULL(bsp_i2c_init());

    if (io_expander) {
        ESP_LOGI(TAG, "io_expander already initialized");
    } else {
...
{
    ESP_LOGI(TAG, "bsp_display_lcd_init");
    assert(cfg != NULL);
...
{
    ESP_LOGI(TAG, "bsp_display_indev_init");
    if (tp == NULL) {
...
{
    ESP_LOGI(TAG, "bsp_display_start_with_config");
...
static esp_err_t bsp_init_main_button(void *param)
{
    ESP_LOGI(TAG, "bsp_init_main_button");

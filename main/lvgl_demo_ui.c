...
static void anim_timer_cb(lv_timer_t *timer)
{
    ...
    ESP_LOGI(TAG,"anim_timer_cb %p count=%d", (void *) timer, count);
...
{
    ESP_LOGI(TAG,"example_lvgl_demo_ui entry");

    ESP_LOGI(TAG,"lv_img_create %p", (void *) scr);

    // Create image
    img_logo = lv_img_create(scr);
    ESP_LOGI(TAG,"lv_img_set_src");
    lv_img_set_src(img_logo, &esp_logo);
    ESP_LOGI(TAG,"lv_obj_center");
    lv_obj_center(img_logo);

    // Create arcs
    ESP_LOGI(TAG,"creating arcs");
    for (size_t i = 0; i < sizeof(arc) / sizeof(arc[0]); i++) {
    ...
    ESP_LOGI(TAG,"creating animation timer");
    ...
    ESP_LOGI(TAG,"example_lvgl_demo_ui exit");
}
